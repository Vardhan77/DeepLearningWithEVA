...
Create an array of transitions and select randomly form the set of samples.

### **Step2**: Define Actor Model
The actor model consists of a flow of linear layers. the model accepts the state dimensions as input and lets out the actions,

### **Step3**: Define Critic Model
The Critic Model , we include 2 of them. Since we need to find the state dimensions which tell us how many variables are in a state. And we also action_dims-how many actions of input can be take.

### **Step4**: Get Random sample from Experience Replay Memory
Get RAndom samples from the initially created data.
